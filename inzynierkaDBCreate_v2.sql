...
/* Created on:     2017-12-20 18:51:02                          */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('Urzadzenie') and o.name = 'FK_URZADZEN_ZAJMUJE S_SERWISAN')
alter table Urzadzenie
   drop constraint "FK_URZADZEN_ZAJMUJE S_SERWISAN"
go
...
if exists (select 1
           ...
           where  id    = object_id('Urzadzenie')
            ...
            and   indid > 0
            and   indid < 255)
   drop index Urzadzenie."zajmuje siê_FK"
go
...
/*==============================================================*/
create index "zajmuje siê_FK" on Urzadzenie (
serwisant_id ASC
)
go
...
alter table Urzadzenie
   add constraint "FK_URZADZEN_ZAJMUJE S_SERWISAN" foreign key (serwisant_id)
      references Serwisant (serwisant_id)
go
